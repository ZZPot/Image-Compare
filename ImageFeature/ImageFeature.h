...
struct image_compare_result
{
	cv::Mat hist;
	cv::Mat pat;
	cv::Mat color_diff; // by first color of initial cells
};
...
public:
	...
	void Set(cv::Mat img);
	image_compare_result CompareFile(std::string img_file);
	image_compare_result CompareImg(cv::Mat img);
	image_compare_result Compare(const image_features& features);
protected:
