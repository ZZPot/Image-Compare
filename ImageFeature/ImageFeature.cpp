...
void image_feature::Set(cv::Mat img, cv::Rect rect)
{
	...
	hist = GetHist(img(subrect), CS_BGR);
}
...
void image_features::Set(cv::Mat img)
{
	...
	for(unsigned i = 0; i < HOR_PARTS * VER_PARTS; i++)
	{
		...
		_features[i].Set(img, roi_rect);
	}
}
image_compare_result image_features::CompareFile(std::string img_file)
{
	return CompareImg(cv::imread(img_file));
}
image_compare_result image_features::CompareImg(cv::Mat img)
{
	return Compare(image_features(img));
}
image_compare_result image_features::Compare(const image_features& features)
{
	image_compare_result res;
	res.hist.create(VER_PARTS, HOR_PARTS, CV_64FC1);
	res.color_diff.create(VER_PARTS, HOR_PARTS, CV_64FC1);
	for(unsigned i = 0; i < _features.size(); i++)
	{
		res.hist.at<double>(i / HOR_PARTS, i % HOR_PARTS) = 
			cv::compareHist(_features[i].hist, features._features[i].hist, cv::HISTCMP_BHATTACHARYYA);
		cv::Scalar scolor_0 = _features[i].dom_colors[0];
		cv::Scalar scolor_1 = features._features[i].dom_colors[0];
	/*	cv::Vec3i color_0(scolor_0[0], scolor_0[1], scolor_0[2]);
		cv::Vec3i color_1(scolor_1[0], scolor_1[1], scolor_1[2]);
		res.color_diff.at<double>(i / HOR_PARTS, i % HOR_PARTS) = 
			GetCIE76Dist(color_0, color_1, CS_BGR);*/
		res.color_diff.at<double>(i / HOR_PARTS, i % HOR_PARTS) = cv::norm(scolor_0, scolor_1);
	}
	res.color_diff /= 441; // sqrt(3*255^2)
	cv::Mat orig_pats = DrawPats(_features);
	cv::Mat comp_pats = DrawPats(features._features);
	cv::Size max_size(std::max(orig_pats.cols, comp_pats.cols), std::max(orig_pats.rows, comp_pats.rows));
	cv::resize(orig_pats, orig_pats, max_size, 0, 0, cv::INTER_NEAREST);
	cv::resize(comp_pats, comp_pats, max_size, 0, 0, cv::INTER_NEAREST);
	cv::absdiff(orig_pats, comp_pats, res.pat);
	res.pat /= max_size.height * max_size.width * 255;
	return res;
}
